...
struct ADSB1090Handler : RTLSDR::IDataHandler, ADSB::IDataProvider
{
    ...
    ADSB1090Handler(std::shared_ptr<ADSB::TrafficManager> trafficManager, RTLSDR::IDeviceSelector const* selector, Source sourceId) :
    ...
    virtual void OnDeviceStatusChanged(bool /* available */) override {}
    ...
    TryCreate(std::shared_ptr<TrafficManager> trafficManager, RTLSDR::IDeviceSelector const* selector, Source sourceId)
    {
    ...
    Source            _sourceId{Source::ADSB1090};
    /* Statistics */
...
static size_t modesMessageLenByType(int type)
{
    ...
    else return Message::ShortMessageBits;
}
...
AirCraftImpl& ADSB1090Handler::_interactiveReceiveData(Message* mm)
{
    ...
    else if (mm->msgtype == 17)
    {
        ...
        else if (mm->metype >= 9 && mm->metype <= 18)
        {
            ...
            if (mm->fflag)
            {
                a.cprOddLat  = (int32_t{mm->raw_latitude});
                a.cprOddLon  = (int32_t{mm->raw_longitude});
                a.cprOddTime = (decltype(now){now});
            }
            else
            {
                a.cprEvenLat  = (int32_t{mm->raw_latitude});
                a.cprEvenLon  = (int32_t{mm->raw_longitude});
